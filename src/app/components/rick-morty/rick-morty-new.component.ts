...
@Component({
  ...
  template: `
    <div class="rick-morty-container">
      ...
      <div class="page-header">
        ...
        <button mat-raised-button color="primary" (click)="addCharacter()" class="add-button">
  ...
  styles: [`
    ...
    /* Estilos globales para el modal de detalles */
    ::ng-deep .character-details-dialog {
      border-radius: 20px !important;
      overflow: hidden;
    }

    ::ng-deep .character-details-dialog .mat-mdc-dialog-container {
      border-radius: 20px;
      padding: 30px;
    ...
    ::ng-deep .character-details-dialog .character-details-container {
      background: rgba(255, 255, 255, 0.95);
      backdrop-filter: blur(10px);
      border-radius: 15px;
      padding: 25px;
...
export class RickMortyComponent implements OnInit {
  ...
  addCharacter() {
    const dialogRef = this.dialog.open(CharacterFormComponent, {
      width: '700px',
      maxHeight: '90vh',
      data: { character: null, isEdit: false },
      panelClass: 'character-form-dialog',
      disableClose: false
    });

    dialogRef.afterClosed().subscribe((result: any) => {
      if (result) {
        this.rickMortyService.addCharacter(result).subscribe(() => {
          this.snackBar.open('Personaje agregado exitosamente', 'Cerrar', {
            duration: 3000
          });
          this.loadCharacters();
        });
  ...
  editCharacter(character: any) {
    const dialogRef = this.dialog.open(CharacterFormComponent, {
      width: '700px',
      maxHeight: '90vh',
      data: { character: { ...character }, isEdit: true },
      panelClass: 'character-form-dialog',
      disableClose: false
    });
    ...
    dialogRef.afterClosed().subscribe((result: any) => {
      if (result) {
        this.rickMortyService.updateCharacter({ ...result, id: character.id }).subscribe(() => {
          this.snackBar.open('Personaje actualizado exitosamente', 'Cerrar', {
            duration: 3000
          });
          this.loadCharacters();
        });
  ...
  viewCharacter(character: any) {
    const dialogRef = this.dialog.open(CharacterDetailsComponent, {
      ...
      panelClass: 'character-details-dialog'
    });
  ...
  deleteCharacter(character: any) {
    // Simulamos un diálogo de confirmación
    if (confirm(`¿Estás seguro de que quieres eliminar a ${character.name}?`)) {
      this.rickMortyService.deleteCharacter(character.id).subscribe(() => {
        this.snackBar.open('Personaje eliminado exitosamente', 'Cerrar', {
          duration: 3000
        });
        this.loadCharacters();
      });
